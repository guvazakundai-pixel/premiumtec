    ...
    <main className="min-h-screen bg-white pt-24 pb-20">
      <div className="max-w-4xl mx-auto px-6">
        <div className="flex items-center justify-between mb-8">
          <div>
            <Link href="/" className="inline-flex items-center gap-2 text-xs text-[#888888] hover:text-black transition-colors mb-3">
              <ArrowLeft size={14} /> Back to Store
            </Link>
            <h1 className="text-3xl md:text-4xl font-bold tracking-tight text-black">Shopping Cart</h1>
          </div>
          {items.length > 0 && (
            <button onClick={handleClear} className="text-xs text-[#888888] hover:text-red-500 transition-colors uppercase tracking-wider">
              Clear All
            </button>
          )}
        </div>
        ...
        {items.length === 0 ? (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center py-20">
            ...
            <p className="text-[#888888] text-sm mb-1">{cleared ? 'Cart cleared' : 'Your cart is empty'}</p>
            <p className="text-[#AAAAAA] text-xs mb-6">Add some premium devices to get started.</p>
          ...
          <div className="space-y-4">
            {items.map((item, i) => (
              ...
              >
                <div className="w-16 h-16 rounded-xl bg-[#F3F3F3] flex items-center justify-center text-[#888888] text-[9px] shrink-0">
                  {item.name.split(' ').slice(0, 2).join(' ')}
                </div>

                <div className="flex-1 min-w-0">
                  <div onClick={() => setSelectedProduct(item)} className="cursor-pointer">
                    <h3 className="text-sm font-medium text-[#4B4B4B] hover:text-black transition-colors truncate">{item.name}</h3>
                  </div>
                  <p className="text-xs text-[#888888] mt-0.5">${item.price.toLocaleString()} each</p>
                </div>

                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-2 rounded-full border border-[#E5E5E5] px-2 py-1">
                    <button onClick={() => updateQty(item.id, item.qty - 1)} className="p-0.5 text-[#888888] hover:text-black transition-colors">
                      <Minus size={12} />
                    </button>
                  ...
                  <button onClick={() => removeItem(item.id)} className="p-2 text-[#888888] hover:text-red-500 transition-colors">
                    <Trash2 size={14} />
                  </button>
                </div>
              </motion.div>
            ...
            >
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
                <div>
                  ...
                  <p className="text-xs text-[#888888] mt-1">{totalItems} item{totalItems !== 1 ? 's' : ''}</p>
                </div>
              ...
              <div className="border-t border-[#E5E5E5] pt-4">
                <div className="flex flex-wrap items-center gap-3 text-xs text-[#888888]">
                  ...
                  {['EcoCash', 'Innbucks', 'Visa', 'Mastercard', 'ZIPIT'].map((p) => (
                    <span key={p} className="px-2 py-1 rounded border border-[#E5E5E5] text-[#888888]">{p}</span>
                  ))}
                </div>
                <p className="text-xs text-[#AAAAAA] mt-3">Free delivery within Harare. Nationwide shipping across Zimbabwe.</p>
              </div>
            </motion.div>
          </div>
        )}
